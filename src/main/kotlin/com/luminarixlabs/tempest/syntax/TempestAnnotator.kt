...
class TempestAnnotator : Annotator {
    ...
    private fun processComments(element: PsiElement, holder: AnnotationHolder, text: String, excludedRanges: MutableSet<IntRange>) {
    ...
    private fun processInterpolations(element: PsiElement, holder: AnnotationHolder, text: String, excludedRanges: Set<IntRange>) {
        ...
        while (safeMatcher.find()) {
            ...
            createAnnotation(holder, startOffset, startOffset + 2, TempestTokens.SAFE_INTERPOLATION_KEY, "Safe interpolation start")
            
            // Highlight the closing braces
            createAnnotation(holder, endOffset - 2, endOffset, TempestTokens.SAFE_INTERPOLATION_KEY, "Safe interpolation end")
            
            // Highlight the content (excluding the braces)
            val contentStart = startOffset + 2
            val contentEnd = endOffset - 2
            if (contentStart < contentEnd) {
                createAnnotation(holder, contentStart, contentEnd, TempestTokens.INTERPOLATION_CONTENT_KEY, "Interpolation content")
            }
        }
        ...
        while (unsafeMatcher.find()) {
            ...
            // Highlight the opening braces
            createAnnotation(holder, startOffset, startOffset + 3, TempestTokens.UNSAFE_INTERPOLATION_KEY, "Unsafe interpolation start")
            
            // Highlight the closing braces
            createAnnotation(holder, endOffset - 3, endOffset, TempestTokens.UNSAFE_INTERPOLATION_KEY, "Unsafe interpolation end")
    ...
    private fun processAttributes(element: PsiElement, holder: AnnotationHolder, text: String, excludedRanges: Set<IntRange>) {
        ...
        while (matcher.find()) {
            ...
            if (value != null) {
                val valueStart = text.indexOf("\"", nameEnd - element.textRange.startOffset) + element.textRange.startOffset
                val valueEnd = text.indexOf("\"", valueStart - element.textRange.startOffset + 1) + element.textRange.startOffset + 1
                if (valueStart >= 0 && valueEnd > valueStart) {
                    createAnnotation(holder, valueStart, valueEnd, TempestTokens.ATTRIBUTE_VALUE_KEY, "Tempest attribute value")
                }
            }
        }
    }
    
    private fun processConditionals(element: PsiElement, holder: AnnotationHolder, text: String, excludedRanges: Set<IntRange>) {
        ...
        while (matcher.find()) {
            ...
            createAnnotation(holder, conditionalStart, conditionalEnd, TempestTokens.CONDITIONAL_KEY, "Tempest conditional")
            
            // Highlight the value if present
            val value = matcher.group(3)
            if (value != null) {
                val valueStart = text.indexOf("\"", conditionalEnd - element.textRange.startOffset) + element.textRange.startOffset
                val valueEnd = text.indexOf("\"", valueStart - element.textRange.startOffset + 1) + element.textRange.startOffset + 1
                if (valueStart >= 0 && valueEnd > valueStart) {
                    createAnnotation(holder, valueStart, valueEnd, TempestTokens.ATTRIBUTE_VALUE_KEY, "Tempest conditional value")
                }
            }
        }
    }
    
    private fun processLoops(element: PsiElement, holder: AnnotationHolder, text: String, excludedRanges: Set<IntRange>) {
        ...
        while (matcher.find()) {
            ...
            val value = matcher.group(3)
            if (value != null) {
                val valueStart = text.indexOf("\"", loopEnd - element.textRange.startOffset) + element.textRange.startOffset
                val valueEnd = text.indexOf("\"", valueStart - element.textRange.startOffset + 1) + element.textRange.startOffset + 1
                if (valueStart >= 0 && valueEnd > valueStart) {
                    createAnnotation(holder, valueStart, valueEnd, TempestTokens.ATTRIBUTE_VALUE_KEY, "Tempest loop value")
                }
            }
        }
    }
